...
TPZString TPZSimulation :: writeSimulationStatus()
{
   ...
   buffer += TPZString(" Traffic Pattern         = ") + m_TrafficPattern->asString() +
	     ...
	     buffer += TPZString("***************************  Power  Consumption ******************* \n");
             ...
             double m_overallDynamicPower =  m_goldenCounterDynamicPower*m_Clock/m_frequency*nodes \
                                             + m_perDCDynamicPower/m_frequency*m_Network->getEventCount( TPZNetwork::SWTraversal);

             double m_overallStaticPower = (m_goldenCounterLeakage + m_perDCLeakage*4)*m_Clock/m_frequency*nodes;
